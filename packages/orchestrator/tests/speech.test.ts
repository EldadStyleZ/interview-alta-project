...
import { createASRProvider, AsrStub, type ASRConfig } from '../src/speech/asr';
import { createTTSProvider, TtsStub, type TTSConfig } from '../src/speech/tts';
...
describe('Speech Adapters', () => {
  describe('ASR Stub', () => {
    it('generates partial transcripts with confidence', async () => {
      ...
      const partials: Array<{ text: string; confidence: number; is_final: boolean }> = [];
  ...
  describe('TTS Stub', () => {
    it('generates audio chunks from text stream', async () => {
      ...
      const chunks: Array<{ format: string; sample_rate: number; data: Buffer }> = [];
